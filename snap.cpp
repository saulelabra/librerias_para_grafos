...
void add_vertex(int value, PNGraph  &g) {
    g.AddNode(value);
}

void add_edge(int a, int b, int weight, PNGraph &g) {
    g.AddEdge(a,b);
    TNEANet::TEdgeI EI = g->GetEI(a,b);
    g.AddIntAttrDatE(EI, weight, "weight");
}

void delete_vertex(int value, PNGraph &g) {
    g->DelNode(value);
}

void delete_edge(int a, int b, PNGraph &g) {
    g->DelEdge(a,b);
}
...
int main(){ //Example Program
    ...
    add_edge(14, 13, 12, &g);
    /*
    add_vertex(g); //Here we insert a vertex
    add_edge(1,5,3,g); //Here we insert an edge from vertex 1 to 5 with weight 3
    delete_vertex(15,g); //Here we delete the vertex 15
    delete_edge(1,5,g); //Here we delete the edge going from Vertex 1 to 5.
    */
